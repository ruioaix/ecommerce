          ...
          <div class="entry-photo-slide">
              <?php if(count($row->field_field_blog_multimedia) == 1) : ?>
                  <?php if($row->field_field_blog_multimedia[0]['rendered']['#file']->type == 'image') : ?>
                      <img src="<?php print image_style_url('blog_thumbnail_455x200', $row->field_field_blog_multimedia[0]['rendered']['#file']->uri); ?>" alt="">
                  <?php elseif($row->field_field_blog_multimedia[0]['rendered']['#file']->type == 'video') : ?>
                      <?php
                          if($row->field_field_blog_multimedia[0]['rendered']['#file']->filemime == "video/vimeo") :
                              ...
                              print '<iframe src="http://player.vimeo.com/video/'. $uri[1].'?title=0&amp;byline=0&amp;portrait=0&amp;color=ec155a" height="200"></iframe>';
                          else :
                              $uri = explode('v/',$row->field_field_blog_multimedia[0]['rendered']['#file']->uri);
                              print '<iframe src="http://www.youtube.com/embed/'. $uri[1].'?feature=oembed" height="200"></iframe>';
                          endif;
                      ?>
              ...
              <?php else : ?>
                  <?php for($i=0; $i < count($row->field_field_blog_multimedia); $i++) : ?>
                      <?php if($row->field_field_blog_multimedia[$i]['rendered']['#file']->type == 'image') : ?>
                          <img src="<?php print image_style_url('blog_thumbnail_455x200', $row->field_field_blog_multimedia[$i]['rendered']['#file']->uri); ?>" alt="">
                      <?php endif; ?>
                  <?php endfor; ?>
              <?php endif; ?>
          </div>
